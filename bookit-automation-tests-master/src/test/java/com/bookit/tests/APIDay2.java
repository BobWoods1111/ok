...
public class APIDay2 {
	 ...
	 */
	String baseurl = "http://107.22.128.211:1000/ords/hr";

	@Test
	public void employeesWithParams() {

		given().accept(ContentType.JSON).and().params("limit", 100).when().get(baseurl + "/employees/").then()
	...
	public void testWithPathParams() {
		given().accept(ContentType.JSON).and().pathParam("id", 110).when().get(baseurl + "/employees/{id}").then()
				.statusCode(200).and().assertThat().contentType(ContentType.JSON).and()
	...
	public void testWithJsonPath() {
		...
		Response response = given().accept(ContentType.JSON).and().pathParam("empId", 110).when()
	...
	/*
	 * Given Accept type is JSON When I send a Get request to url/employees/140 Then
	 * status code is 200 And response content should be json all data should be
	 * returned
	 */
	...
	public void testWithJSONtoMap() {
		...
		Response response = given().accept(ContentType.JSON).and().when().get(baseurl + "/employees/140");
	...
	public void convertJSONtoListofMaps() {
		...
		Response response = given().accept(ContentType.JSON).when().get(baseurl + "/departments");
	...
	// TASK
	/*
	 * Given Content type is JSON And Limit is 10 When I send the GET request to
	 * url/regions the status code must be 200 The I should see following data 1
	 * Europe 2 Americas 3 Asia 4 Middle East and Africa
	 */

	@Test
	public void regionTaskV1() {

		given().accept(ContentType.JSON).and().params("limit", 10).when().get(baseurl + "/regions").then()
				.statusCode(200).and().assertThat()
				.body("items.region_name", Matchers.hasItems("Europe", "Americas", "Asia", "Middle East and Africa"));
